...
///
/// Flag to indicate whether the code generated for the CPU should be
/// vectorized.
...
///
/// Flag to indicate the order in which SLP-vectorized instructions should be
/// arranged. True to reorder them based on a depth-first search, false to
/// reorder them based on a breadth-first search.
extern Option<bool> slpReorderInstructionsDFS;

///
/// Flag to indicate whether duplicate elements are allowed in vectors during
/// SLP graph building. True to keep growing the graph when duplicates are
/// encountered in a vector, false to stop growing.
extern Option<bool> slpAllowDuplicateElements;

///
/// Flag to indicate if elements with different topological depths are allowed
/// in vectors during SLP graph building. True to allow mixing, false to stop
/// growing the graph at that vector if mixed topological depths occur.
extern Option<bool> slpAllowTopologicalMixing;

///
/// Flag to indicate if XOR chains should be used to compute look-ahead scores
/// instead of Porpodas's algorithm. True to use XOR chains, false to use
/// Porpodas's algorithm.
