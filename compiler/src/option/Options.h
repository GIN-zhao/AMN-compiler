...
///
/// Represents a concrete configuration, i.e., it stores OptionValues for all
...
public:
  ...
  /// \return The value of the Option. Will throw if the option was not given in
  /// the configuration.
  OptValue &get(const std::string &key) const { return *config.at(key); }

  /// Check if an Option was specified in this Configuration.
  /// \param key Identifier of the Option.
  /// \return true if the Option was specified, false otherwise.
  bool hasOption(const std::string &key) const { return config.count(key); }
...
public:
  ...
  /// \return None if the key did not match the identifier, otherwise the
  /// OptionValue parsed from the string.
  virtual llvm::Optional<std::unique_ptr<OptValue>>
  parse(const std::string &key, const std::string &value) = 0;

  /// Check if this option was specified in the Configuration.
  /// \param config Configuration.
  /// \return true if an option with the matching identifier was present in the
  /// configuration.
...
public:
  ...
  void initialize(Opt *opt) { modified = opt; }
...
///
/// Global container for all options. Statically initializes appropriate
/// storage. All options should be specified as global variables and are then
/// automatically inserted into this container.
class Options {

public:
  /// Insert an option into the container. Does not assume ownership of the
  /// option. \param key Identifier of the option. \param opt Pointer to the
  /// option. \param mods Option modifiers attached to the given option.
  ...
  parse(const std::map<std::string, std::string> &input) {
  ...
  /// Register a new modifier instance in this container, assumes ownership of
  /// the modifier. \tparam Modifier The type of the modifier. \param mod
  /// OptModifier \return A non-owning pointer to the modifier.
  ...
  /// Collect parsed command-line options and store into a container, which can
  /// be used internally. \param app Reference from which the options will be
  /// extracted.
...
public:
  ...
  explicit Option(std::string k,
                  std::initializer_list<OptModifier *> modifier = {}) noexcept
      : Opt{std::move(k)} {
  ...
  /// \param modifier Constraints to attach to this option.
  Option(std::string k, Value defaultVal,
         std::initializer_list<OptModifier *> modifier = {}) noexcept
      : Opt{std::move(k)} {
    hasDefault = true;
  ...
  /// \return llvm::None if the string identifier did not match, otherwise the
  /// parsed value.
  llvm::Optional<std::unique_ptr<OptValue>>
  parse(const std::string &key, const std::string &value) override {
    if (key != keyName) {
      SPDLOG_WARN("Identifier did not match this option!");
      return llvm::None;
    }
    std::unique_ptr<OptValue> result = std::make_unique<OptionValue<Value>>(
        detail::OptionParsers::parse<Value>(value));
    return result;
  }

  /// Retrieve the value specified for this Option from the Configuration.
  /// \param config Configuration.
  /// \return The explicitly specified value. If none was given, the default
  /// value. In case this option does not have a default value, throws an error.
  ...
  /// Retrieve the value specified for this Option from the Configuration or an
  /// alternative value. \param config Configuration. \param elseValue Value to
  /// use if the option was not specified. Supersedes the default-value of the
  /// option. \return The explicitly specified value or the given alternative
  /// value.
...
///
/// Special Option that takes a number of predefined, enumerated values as
/// input.
///
...
public:
  ...
  EnumOpt(std::string k, std::initializer_list<OptionEnumValue> options,
          std::initializer_list<OptModifier *> modifier = {})
      : Option<int>{k, modifier} {
    for (auto &o : options) {
  ...
  /// \tparam D Type of the default value. It must be possible to convert this
  /// to int (e.g. enum). \param k String identifier for this option. \param
  /// defaultVal Default value. \param options Allowed values for this option.
  /// \param modifier Constraints to attach to this option.
  template <typename D>
  EnumOpt(std::string k, D defaultVal,
          std::initializer_list<OptionEnumValue> options,
          std::initializer_list<OptModifier *> modifier = {})
      : Option<int>{k, int(defaultVal), modifier} {
    for (auto &o : options) {
      enumValues.emplace(detail::OptionParsers::toLowerCase(o.name), o);
    }
  }
  ...
  parse(const std::string &key, const std::string &value) override {
    ...
    if (!enumValues.count(v)) {
      ...
      SPDLOG_WARN("Specified value {} did not match any of the possible values",
                  v);
      return llvm::None;
    }
...
public:
  ...
  Depends(Option<OptVal> &opt, Value depend) : depOpt{opt}, depVal{depend} {
    static_assert(std::is_constructible<Value, OptVal>::value,
                  "Incompatible type for dependency!");
  }
...
static Depends<OptVal, Value> *depends(Option<OptVal> &opt, Value depend) {
  return Options::registerModifier(
      std::make_unique<Depends<OptVal, Value>>(opt, depend));
}
