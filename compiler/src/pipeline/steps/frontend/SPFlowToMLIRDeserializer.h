...
///
/// Step to read a serialized SPN query + graph from a binary file and translate
/// it to an MLIR module, mainly using the HiSPN dialect.
class SPFlowToMLIRDeserializer
    : public StepSingleInput<SPFlowToMLIRDeserializer, BinarySPN>,
...
private:
  ...
  void deserializeJointQuery(JointProbability::Reader &&query, int batchSize,
                             mlir::spn::high::error_model errorKind,
                             double maxError);
  ...
  mlir::spn::high::ProductNode
  deserializeProduct(ProductNode::Reader &&product);

  mlir::spn::high::HistogramNode
  deserializeHistogram(HistogramLeaf::Reader &&histogram);

  mlir::spn::high::GaussianNode
  deserializeGaussian(GaussianLeaf::Reader &&gaussian);

  mlir::spn::high::CategoricalNode
  deserializeCaterogical(CategoricalLeaf::Reader &&categorical);
