...
public:
  ...
  /// \param _deleteOnExit Flag to indicate whether this file should be deleted
  /// on exit from the compiler. Defaults to false.
  explicit File(const std::string &fileName, bool _deleteOnExit = false)
      : fName{fileName}, deleteOnExit{_deleteOnExit} {}
...
File<Type> FileSystem::createTempFile(bool deleteTmpOnExit) {
  ...
  // Use the mkstemps function from the Posix standard to create a temporary
  // files with file suffix.
